...
info and document field storage info and vector term info (like term frequency etc..)  , Here in inverted index it contians the map of terms to docIDs for example. (Here one thing to note is es stores the term to docIds mapping segments and during read query its going to scan the segments and fetches the docIds and aggregate them , There was no global term to docIds map instead eventually smaller segments will be merged to larger segments as an optimization)
